...
- Please go to `Settings` -> [`Playback Sources`](betterlyrics://settings/playbacklib) -> `Lyrics Search Strategy` and select `Best Match`.
...
Go to `Settings` -> [`Playback Sources`](betterlyrics://settings/playbacklib) > Disable `Lyrics Timeline Sync` or increase the `Lyrics Timeline Sync Threshold`.

### How to set up local lyrics sources?

**Step 1: Add Folder**
Go to `Settings` -> [`Media Library`](betterlyrics://settings/medialib), click the `Add` button, add the folder where you store lyrics (or music files containing embedded lyrics), and ensure the switch on the right is set to **"On"**.
